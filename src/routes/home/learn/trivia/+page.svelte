<script>
	import { triviaQuestions, randomTrivia } from '$lib/data/trivia';
	import { ProgressBar, RangeSlider, RadioGroup } from '@skeletonlabs/skeleton';
	import { fade, fly, scale } from 'svelte/transition';
	const correctSound = new Audio('/trivia_assets/score-1.mp3');
	const wrongSound = new Audio('/trivia_assets/wrong-1.mp3');
	const gameOverSound = new Audio('/trivia_assets/gameover-1.wav');
	...
	let CURR_QUESTION = triviaQuestions[randomTrivia()];
	...
	function makeGuess(guess, index) {
		...
		setTimeout(() => {
			...
			CURR_QUESTION = triviaQuestions[randomTrivia()];
		}, '2500');
	}
...
{#if CURR_GAME_STATE === 'READY'}
	...
	<div class="w-full space-y-4 p-4" in:fly={{ x: -120, delay: 100 }}>
		<h3 class="w-full text-3xl font-bold">Bible Trivia</h3>
